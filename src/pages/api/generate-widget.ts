...
type MapArgs = { locations: Array<{ name: string; lat: number; lon: number; color?: string }> };
...
type WidgetResponse =
  | { type: "line" | "bar" | "pie"; title: string; data: ChartArgs["data"]; source: "document" | "gemini" }
  | { type: "map"; data: MapArgs["locations"] }
  | { type: "image"; src: string }
  | { type: "video"; src: string }
  | { type: "weather"; location?: string; coordinates: [number, number] | "current" }
...
// Aggregate helper for document data
function aggregateDocData(
  ...
  const groupField = groupBy && doc.fields.includes(groupBy) ? groupBy : doc.fields[0];
  ...
  return Object.entries(groups).map(([label, values]) => {
    if (aggregation === "sum") return { label, value: values.reduce((a, b) => a + b, 0) };
    if (aggregation === "avg") return { label, value: values.reduce((a, b) => a + b, 0) / values.length };
    return { label, value: values.length };
  });
}

export default async function handler(req: NextApiRequest, res: NextApiResponse<WidgetResponse | WidgetResponse[]>) {
  ...
  try {
    ...
    if (fnCall.name.startsWith("create_")) {
      ...
      if (args.source === "compare" && args.filename && sessionStore[args.filename]) {
        ...
        const dataFromDoc = aggregateDocData(doc, args.groupBy, args.metric, args.aggregation);
        return res.status(200).json([
          { type, title: args.title || `Chart from ${args.filename}`, data: dataFromDoc, source: "document" },
          { type, title: args.title || "Chart from Gemini", data: args.data, source: "gemini" },
        ]);
      }

      if (args.source === "document" && args.filename && sessionStore[args.filename]) {
        const doc = sessionStore[args.filename];
        const data = aggregateDocData(doc, args.groupBy, args.metric, args.aggregation);
        return res.status(200).json({ type, title: args.title || `Chart of ${args.groupBy || doc.fields[0]}`, data, source: "document" });
      }

      return res.status(200).json({ type, title: args.title, data: args.data, source: "gemini" });
    }

    if (fnCall.name === "get_weather") {
      const args = fnCall.args as WeatherArgs;
      if (typeof args.lat === "number" && typeof args.lon === "number") {
        return res.status(200).json({ type: "weather", coordinates: [args.lat, args.lon] });
      } else if (args.city) {
        return res.status(200).json({ type: "weather", location: args.city, coordinates: [0, 0] });
      } else {
        return res.status(200).json({ type: "weather", coordinates: "current" });
      }
    }
    ...
    if (fnCall.name === "create_image") {
      ...
      return res.status(200).json({
        type: "image",
        src: `https://picsum.photos/seed/${encodeURIComponent(args.prompt || "default")}-${Math.floor(Math.random() * 1000)}/800/600`,
      });
    }
    ...
    if (fnCall.name === "create_video") {
      ...
      return res.status(200).json({
        type: "video",
        src: args.query || "https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4",
      });
    }
    ...
    if (fnCall.name === "analyze_document") {
      ...
      if (args.question) {
        const qaPrompt = `You are analyzing "${args.filename}". Question: ${args.question}\nFields: ${JSON.stringify(doc.fields)}\nPreview: ${JSON.stringify(doc.preview)}`;
